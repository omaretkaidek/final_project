const { body, param } = require("express-validator");
const knex = require("knex")(require("../../../knexfile"));

const nameValidator = body("name")
  .isString()
  .withMessage("Name must be a string")
  .notEmpty()
  .withMessage("Name is required");

const establishDateValidator = body("establishDate")
  .isDate()
  .withMessage("Establish Date must be a valid date")
  .notEmpty()
  .withMessage("Establish Date is required");

const stillWorkingValidator = body("stillWorking")
  .isBoolean()
  .withMessage("Still Working must be a boolean")
  .notEmpty()
  .withMessage("Still Working is required");

const idValidator = param("id")
  .isInt()
  .withMessage("ID must be an integer")
  .custom(async (value) => {
    const publisher = await knex("publishers")
      .where({ publisherId: value })
      .first();
    if (!publisher) {
      return Promise.reject("Publisher ID does not exist in the database");
    }
  });

const idValidation = [idValidator];

const createPublisherValidationRules = [
  nameValidator,
  establishDateValidator,
  stillWorkingValidator,
];

module.exports = {
  createPublisherValidationRules,
  idValidation,
};
